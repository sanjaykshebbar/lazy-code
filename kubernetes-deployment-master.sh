...
# lazy-code-stub: true
# =============================================================================
# MOVED - this script now lives at:  Linux/install-kubernetes-master.sh
#
...
# =============================================================================
echo "NOTE: this path has moved to 'Linux/install-kubernetes-master.sh' - forwarding to the new location..." >&2

_tmp="$(mktemp)" || exit 1
if ! curl -fsSL "https://raw.githubusercontent.com/sanjaykshebbar/lazy-code/main/Linux/install-kubernetes-master.sh" -o "$_tmp"; then
    echo "ERROR: could not fetch Linux/install-kubernetes-master.sh" >&2
    rm -f "$_tmp"
    exit 1
fi
chmod +x "$_tmp"
