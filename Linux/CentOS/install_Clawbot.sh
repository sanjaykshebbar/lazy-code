...
# lazy-code-stub: true
# =============================================================================
# MOVED - this script now lives at:  Linux/CentOS/install-clawbot.sh
#
...
# =============================================================================
echo "NOTE: this path has moved to 'Linux/CentOS/install-clawbot.sh' - forwarding to the new location..." >&2

_tmp="$(mktemp)" || exit 1
if ! curl -fsSL "https://raw.githubusercontent.com/sanjaykshebbar/lazy-code/main/Linux/CentOS/install-clawbot.sh" -o "$_tmp"; then
    echo "ERROR: could not fetch Linux/CentOS/install-clawbot.sh" >&2
    rm -f "$_tmp"
    exit 1
fi
chmod +x "$_tmp"
