...
# lazy-code-stub: true
# =============================================================================
# MOVED - this script now lives at:  Intune/macOS/install-cocoapods.sh
#
...
# =============================================================================
echo "NOTE: this path has moved to 'Intune/macOS/install-cocoapods.sh' - forwarding to the new location..." >&2

_tmp="$(mktemp)" || exit 1
if ! curl -fsSL "https://raw.githubusercontent.com/sanjaykshebbar/lazy-code/main/Intune/macOS/install-cocoapods.sh" -o "$_tmp"; then
    echo "ERROR: could not fetch Intune/macOS/install-cocoapods.sh" >&2
    rm -f "$_tmp"
    exit 1
fi
chmod +x "$_tmp"
