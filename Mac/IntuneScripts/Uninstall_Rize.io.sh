...
# lazy-code-stub: true
# =============================================================================
# MOVED - this script now lives at:  Intune/macOS/uninstall-rize-io.sh
#
...
# =============================================================================
echo "NOTE: this path has moved to 'Intune/macOS/uninstall-rize-io.sh' - forwarding to the new location..." >&2

_tmp="$(mktemp)" || exit 1
if ! curl -fsSL "https://raw.githubusercontent.com/sanjaykshebbar/lazy-code/main/Intune/macOS/uninstall-rize-io.sh" -o "$_tmp"; then
    echo "ERROR: could not fetch Intune/macOS/uninstall-rize-io.sh" >&2
    rm -f "$_tmp"
    exit 1
fi
chmod +x "$_tmp"
