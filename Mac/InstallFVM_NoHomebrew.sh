...
# lazy-code-stub: true
# =============================================================================
# MOVED - this script now lives at:  macOS/install-fvm.sh
#
...
# =============================================================================
echo "NOTE: this path has moved to 'macOS/install-fvm.sh' - forwarding to the new location..." >&2

_tmp="$(mktemp)" || exit 1
if ! curl -fsSL "https://raw.githubusercontent.com/sanjaykshebbar/lazy-code/main/macOS/install-fvm.sh" -o "$_tmp"; then
    echo "ERROR: could not fetch macOS/install-fvm.sh" >&2
    rm -f "$_tmp"
    exit 1
fi
chmod +x "$_tmp"
