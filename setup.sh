...
# lazy-code-stub: true
# =============================================================================
# MOVED - this script now lives at:  Linux/setup-ubuntu-host.sh
#
...
# =============================================================================
echo "NOTE: this path has moved to 'Linux/setup-ubuntu-host.sh' - forwarding to the new location..." >&2

_tmp="$(mktemp)" || exit 1
if ! curl -fsSL "https://raw.githubusercontent.com/sanjaykshebbar/lazy-code/main/Linux/setup-ubuntu-host.sh" -o "$_tmp"; then
    echo "ERROR: could not fetch Linux/setup-ubuntu-host.sh" >&2
    rm -f "$_tmp"
    exit 1
fi
chmod +x "$_tmp"
