#!/usr/bin/env bash
# Description: CocoaPods install with full step-by-step logging
# Platform: macOS
###############################################################################
# Author  : Sanjay KS
...
# - Logs complete execution to $HOME/pod_install.log
###############################################################################
...
LOG_FILE="$HOME/pod_install.log"

# Redirect all stdout and stderr to log (and console)
exec > >(tee -a "$LOG_FILE") 2>&1

echo "=================================================="
echo "CocoaPods Installation Started"
echo "Timestamp: $(date)"
echo "=================================================="
...
if [[ ! -d "$OPENSSL_DIR" ]]; then
  ...
  echo "OpenSSL already exists. Skipping build."
fi
...
if [[ ! -d "$LIBYAML_DIR" ]]; then
  ...
  echo "libyaml already exists. Skipping build."
fi
...
if [[ ! -d "$RUBY_DIR" ]]; then
  ...
  echo "Ruby already exists. Skipping build."
fi
...
echo "Configuring RubyGems isolation..."
